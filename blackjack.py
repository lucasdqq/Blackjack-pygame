...
class Game:
    ...
    def check_winner(self):
        if self.user.sum == self.dealer.sum:
            self._result_text = "Empate!"
        elif self.user.sum <= 21 and (len(self.user.cards) == 5 or self.user.sum > self.dealer.sum or self.dealer.sum > 21):
            self._result_text = "Voce venceu!"
            self.win_count += 1
        else:
            self._result_text = "Voce perdeu!"
            self.loss_count += 1
